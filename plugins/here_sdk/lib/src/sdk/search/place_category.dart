...
abstract class PlaceCategory {
  ...
  /// It is available only when when `PlaceCategory` is obtained from `Place` returned by
  /// one of the methods in `SearchEngine`.
  /// That means that when `PlaceCategory` is constructed directly by the client or
  /// the source of the corresponding `Place` is `OfflineSearchEngine`, `name` is always `null`.
