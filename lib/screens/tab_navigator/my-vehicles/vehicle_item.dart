...
class VechicleItem extends StatelessWidget {
  const VechicleItem(
      ...
      required this.index,
      this.isFromAvailRoute});
  final Vehicle vehicle;
  final int index;
  final bool? isFromAvailRoute;
  @override
  Widget build(BuildContext context) {
    // BottomTabController btmTabCtrl = Get.find();ss
    return GetBuilder<BottomTabController>(
      builder: (btmTabCtrl) => GestureDetector(
        behavior: HitTestBehavior.opaque,
        onTap: () {
          if (isFromAvailRoute == true) return;
          btmTabCtrl.makeSelectedCar(index, context);
        },
        child: Container(
          ...
          child: ListTile(
            ...
            trailing: isFromAvailRoute == true
                ? const SizedBox()
                : Wrap(
                    spacing: 8,
                    children: [
