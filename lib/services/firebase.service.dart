...
class FirebaseService {
  ...
  Future<AuthorizationCredentialAppleID> signInWithApple() async {
    ...
    // log(appleProvider.parameters.toString());
    // final credential =
  ...
  Future<GoogleSignInAuthentication?> signInWithGoogle() async {
    ...
    // Once signed in, return the UserCredential
    return googleAuth; //await FirebaseAuth.instance.signInWithCredential(credential);
  }
  ...
  // Future
  updateVehicle(String id, data) async {
  ...
  deleteAccount(BuildContext context) async {
    try {
      ...
      // Handle exceptions
      srvToastAlert
          .toast("Something went wrong while deleting your account".tr);
    }
    // ignore: use_build_context_synchronously
  }
  ...
  getUserFromFirestore(User user) {
  ...
  addFeedBack({String feedback = "", double rating = 5, int index = 0}) {
    ...
    //   "feedback": feedback,
    //   "rating": rating,
    //   "startPosDesp": mapController.startPlace.mainText,
    //   "endPosDesp": mapController.endPlace.mainText,
    //   "startLatLng":
