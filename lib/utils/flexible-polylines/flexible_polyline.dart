...
import 'package:new_trip_start/utils/flexible-polylines/latlngz.dart';
import 'package:new_trip_start/utils/flexible-polylines/converter.dart';
...
class FlexiblePolyline {
  ...
  static List<GeoCoordinates> decode(String? encoded) {
    ...
    final List<GeoCoordinates> results = [];
...
class _Decoder {
  ...
  GeoCoordinates? decodeOne() {
    ...
    if (hasThirdDimension()) {
      ...
      index = zResult.item2;
      return GeoCoordinates(latResult.item1, lngResult.item1);
    }
    return GeoCoordinates(latResult.item1, lngResult.item1);
  }
}
